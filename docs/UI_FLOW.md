...
- 両laneの開始状態（lane位置0）を同時に選択した状態では作成リストへ追加できない。
  既に持っている武器を到達点にすることになるためである
...
- 選択・改善優先を変更すると既存の作成プランは再計算対象になる。変更後は
  「途中採用する状態と改善優先を更新しました。生産計画を再作成してください。」と案内する
- 同一laneから2つ以上の到達点を選択できない。両laneの開始状態の同時選択はDomain
  validationが拒否する
