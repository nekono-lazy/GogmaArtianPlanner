...
- 目標武器Select（検索対象がONかつ完了済みでない目標武器だけを並べる。単一選択。作成リストに登録済みの
  目標武器だけに「登録済み」を表示する。登録済みでも選択でき、単体検索で再検索できる）
- 検索起点（実行中の生産計画がある場合だけ表示）: 現在地点（既定） / 実行中の生産計画の完了後（予測）
- 経路フィルタ: すべて / 通常アーティア経由（新規作成・所持通常の両方） / 既存巨戟から
- 探索量の詳細設定（通常アーティア / 復元ボーナス / スキルの3上限のみ。下記「探索量の上限」）
- 理想品候補1件
- 途中採用する状態の選択（スキル候補 / 復元ボーナス候補）と理想品までの改善優先

1回の検索（Candidate Searchの1 request）は1つの目標武器だけを対象とする。通常の単体検索も、
9.1の一括検索・追加の各検索も同じである。結果フィルタ（すべて / 理想 / 実用 / 近似）は
存在しない。

探索量の上限（Issue #125、[REQUIREMENTS.md](./REQUIREMENTS.md) 14.1）。

- 詳細設定の3入力は「通常アーティア最大進行量」「復元ボーナス最大進行量」「スキル最大進行量」とする。
  ユーザー向け表示では「巨戟最大進行量」「巨戟進行」を使わない（内部名 `maxGogmaAdvance` は変えない）。
  候補カードの操作量サマリも「復元ボーナス進行」、what-if比較も「復元ボーナス進行量」と表示する
- 画面を開いたときの初期値は、設定画面（14）で保存した `AppSettings.candidateSearchDefaults` とする。
  保存値のない新規環境では推奨の初期値 350 / 500 / 1500 になる。画面の基本データと同時に読み込み、
  読み込めなかった場合は推奨の初期値を使い、その旨を検索条件の近くに警告として表示する（検索は止めない）
- 画面で変更した値は、その画面での単体検索と一括検索・追加（9.1）だけに使う。AppSettingsへ自動保存しない。
  画面を開き直すと保存済みの既定値から始まる
- 詳細設定の説明に、初期値が設定画面の既定値であり、ここでの変更は今回の検索だけに使われることを示す
...
- 同じ目標武器に別の候補が作成リストにある場合（[DATA_MODEL.md](./DATA_MODEL.md) 9.4.1 /
  [SEARCH_SPEC.md](./SEARCH_SPEC.md) 10.1、Phase 0-2で実装済み）は、追加せず置換の確認Dialogを出す。
  画面は `BuildListService.addCandidate()` の結果（`added` / `duplicate` / `replacement_required` /
  `legacy_duplicate`）をそのまま扱い、同じ判定を画面側で再実装しない。Presentationは `ui-ux-pro-max` の
  提案を既存のMUI部品・themeへ合わせて次のとおり確定した
  - Dialog題名「作成リストの候補を置き換えますか？」。冒頭に「目標武器: 名前」と「作成リストは目標武器ごとに
    候補を1件だけ登録します。「置き換える」を選ぶと、現在の候補を作成リストから外し、新しい候補を登録します。」
  - 「現在の候補」「新しい候補」の2つの見出し付き区画を、PC（md以上）では横並び、スマートフォンでは縦積み
    （現在 → 新しい）で表示する。区別は見出しの文字で行い、新しい候補には左罫線を補助として付ける（色だけに
    依存しない）。各区画は作成ルートの種類（chip）、起点武器（所持武器を起点にする場合）、完成時の復元ボーナス
    5枠、シリーズ / グループスキル、操作回数、途中採用する状態（スキル / 復元ボーナス）と改善優先を示す。
    現在の候補の設定は「現在の途中採用する状態・改善優先（引き継がれません）」、新しい候補の設定は
    「登録する途中採用する状態・改善優先」と明示する。現在の候補がstaleなら「再検索が必要」chipを付ける。
    Route DTOやIDは表示せず、Candidate / Entryに記録済みの値だけを表示する（RNGを再実行しない）
  - warning Alert「引き継がれない設定」: 「現在の候補で設定している「途中採用する状態」と「改善優先」は、新しい
    候補に引き継がれません。新しい候補には、この画面で選択している内容が登録されます。」
  - actionは「キャンセル」と「置き換える」（通常のprimary button。削除ではなく置換操作なので破壊的な強調は
    しない）。スマートフォンでは全幅・高さ44px以上で縦に並べる。Esc / 背景クリックもキャンセルと同じで、
    何も保存しない。Dialogは `aria-labelledby` / `aria-describedby` を持ち、長い目標武器名・武器名は折り返す
  - 「置き換える」の後に `inspectCandidateReplacement()` を行い、実行中の生産計画を壊す場合だけ16.3の共通
    警告Dialog（操作固有の説明「作成リストの候補を置き換えると、現在の生産計画の前提と一致しなくなります。」）を
    重ねて出す。警告がCandidate置換の確認を兼ねることはない。警告のキャンセルでは何も保存せず、置換確認
    Dialogはそのまま残る。承認時は16.3の承認付きで `replaceCandidate()` を1回呼ぶ。警告が不要なら2つ目の
    Dialogは出さずにそのまま置換する
  - 成功時は「作成リストの候補を置き換えました。」（承認でPlanを破棄した場合は「作成リストの候補を置き換え、
    実行中の生産計画を破棄しました。」）と表示し、画面内の作成リスト状態は旧Entryを除き新Entryを加えた1件に
    なる（2件を並べて表示しない）。候補は「作成リスト: 追加済み」になる
  - Serviceが型付きcodeで拒否した場合（`replacement_target_changed` / `legacy_duplicate_entries` /
    `target_not_found` / `candidate_already_added`）はDialogを閉じ、そのmessageと「ビルドリストを確認する」
    導線を候補の横に表示し、作成リスト状態を読み直す。画面が推測で再試行・再置換することはない
- 同じ目標武器に候補が2件以上ある（既存データ）場合は追加も置換もせず、置換確認Dialogも出さない。候補の横に
  「この目標武器には作成リストに複数の候補が登録されています。作成リストで使用する候補を1件にしてから、
  もう一度操作してください。」と「ビルドリストで整理する」導線を表示する。どの候補を残すかは画面が決めない
...
作成リストに未登録の目標武器を、既存の単体Candidate Searchで1件ずつ順に検索し、見つかった理想品候補を
作成リストへ追加する。Search画面のUI / Application orchestrationであり、Search Domain / Workerの
複数Target contractではない。各検索は通常の単体検索と同じく1 request = 1 Targetで、Targetごとに独立した
`searchRunId` を使う（[SEARCH_SPEC.md](./SEARCH_SPEC.md) 3章）。RNG / Search algorithm / Candidate
semantics / Planner semanticsは変わらない。

- 対象は、単体検索の対象になれる目標武器（検索対象ON、`lifecycleStatus = "active"`）のうち、作成リストに
  同じ `targetWeaponId` のEntryが1件も無いものとする。登録済みかどうかはCandidateのsemantic一致ではなく
  Entryの存在で判定する。staleなEntryを持つ目標武器、同じ目標武器に複数Entryがある（legacy duplicate）
  目標武器も登録済みとして対象外にする。判定authorityは `findBuildListRegisteredTargetIds()`
  （`src/domain/buildList/buildListCardinality.ts`）であり、画面は作成リストの読み取り結果から件数を出す
- 導線は単体の「検索開始」の横の副次ボタン「未登録を一括検索・追加（N件）」とし、Nは現在の対象件数とする。
  0件なら無効化し、「作成リストに未登録の目標武器はありません。」と表示する。単体検索中・一括処理中・
  作成リスト追加 / 置換 / 事前警告 / 目標完了の確認中は開始できない。単体検索と一括処理は同時に実行しない
- 検索条件は開始時点の画面の共通設定（作成ルート、通常アーティア / 復元ボーナス / スキルの探索量上限）を
  全対象に使う。探索量は保存済みの既定値ではなく、その時点で画面に表示されている値（画面で一時変更した値を
  含む）である。目標武器ごとの特別な検索条件も、一括検索専用の既定値もない
- 候補が見つかった目標武器は、単体検索と同じく候補を保存したうえで `BuildListService.addCandidate()` で
  作成リストへ追加する。途中採用する状態は選ばず（両laneとも `null`、理想品まで進む）、改善優先は
  「生産計画に任せる」（`planner`）とする。これは `defaultIntermediateStateSelection()` そのものであり、
  妥協状態を自動選択するものではない。必要な目標武器だけ、後から作成リスト（10章）で途中採用する状態と
  改善優先を変更する。一括処理専用の妥協ルールはない
- 候補が見つからなかった目標武器は作成リストへ追加せず、結果に記録して次へ進む
- 1件の検索・保存・追加が失敗しても一括処理全体は止めず、その目標武器を失敗として記録して次へ進む。
  ただしSearch Worker自体の異常終了（Worker runtime error）では以降の検索もすべて失敗するため、その時点で
  一括処理を止め、以降の目標武器を未処理とする
- 処理中に状態が変わり `addCandidate()` が `duplicate` / `replacement_required` / `legacy_duplicate` を
  返した目標武器は、Serviceの結果をそのまま「追加しなかった」として記録し、次へ進む。一括処理中に自動置換は
  せず、置換確認Dialogも出さない
- キャンセルは現在の検索を既存のWorker cancel contractで止め、以後の目標武器の検索を開始しない。
  キャンセル前に追加した作成リストの項目はそのまま保持し、ロールバックしない。キャンセル後に届いた検索結果は
  保存しない。キャンセル後は単体検索・一括検索を再び実行できる
- 処理中は、現在の目標武器名、「k / N 件目」、完了件数 / 全件数、単体検索の現在phaseと探索ステップ数、
  キャンセル操作を表示する。目標武器ごとに探索量が異なり総work量も事前に確定しないため、全体の進捗率は
  表示しない
- 完了後は候補カードを並べず、コンパクトなサマリを表示する。「作成リストへ追加」「候補なし」「検索失敗」
  「追加しなかった」（と、キャンセル / 中断時の「未処理」）の件数を示し、各区分の目標武器名（失敗は
  message、追加しなかったものは理由）を折りたたみで確認できる。作成リストへの導線を置く
- 一括処理の終了後は作成リストを永続状態から読み直し、Selectの「登録済み」表示と対象件数を同期する
- PCでは2つのボタンを横並び、スマートフォンでは全幅・高さ44px以上で縦に並べる。長い目標武器名は折り返し、
  「登録済み」表示は折り返さず名前の横に置く
...
- 改善優先はsoft preferenceであり、選択した途中状態（hard constraint）より常に下位である
  ことを説明する
- 作成リストは目標武器ごとに候補を最大1件だけ持つ（[DATA_MODEL.md](./DATA_MODEL.md) 9.4.1）。
  同じ目標武器に候補が2件以上ある既存データでは、整理されるまで生産計画の作成と再計画の試算は
  Plannerのfail closed（`duplicate_build_list_entries_for_target` warning）により作成されない（Phase 0-1で実装済み）。
  画面はPlanner操作を独自に無効化せず、このfail closedに委ねる。
  案内（Phase 0-2で実装済み）: どの目標武器がlegacy duplicateかは `findBuildListTargetDuplicates()` の結果で
  決め（staleなEntryも数える）、その目標武器グループの見出し行に「要整理」chipを付け、見出しの直下に
  warning Alert（題名「候補が複数登録されています」、本文「この目標武器には作成リストの候補が複数登録されて
  います。生産計画の作成と再計画の試算には、使用する候補を1件にする必要があります。」「残す候補を確認し、
  不要な候補を「ビルドリストから削除」で削除してください。どの候補を残すかは自動では決めません。」）を出す。
  色だけでなくchipとAlertの文字で示す。すべての候補をそのまま並べ、「おすすめ」や最短・最新・stale状態を
  理由にした強調・自動選択・自動削除をしない。整理は既存の「ビルドリストから削除」（16.3の警告を含む）で行い、
  残りが1件になった時点で再読み込みなしにchipとAlertが消える。constrained re-searchの採用（Phase 0-3までは
  元Entryの横に追加される）で生じたlegacy duplicateも同じ案内になる
...
候補検索（Issue #125、[REQUIREMENTS.md](./REQUIREMENTS.md) 14.1、[DATA_MODEL.md](./DATA_MODEL.md) 13）。

- 「表示設定」と「データ管理」の間に「候補検索」sectionを置き、「通常アーティア最大進行量」
  「復元ボーナス最大進行量」「スキル最大進行量」の3入力（9と同じ表示名・説明）を並べる。PCでは3列、
  スマートフォンでは縦1列とし、横scrollを発生させない
- 説明として、候補検索画面を開いたときの初期値であること、候補検索画面での変更は今回の検索（一括検索・追加を
  含む）だけに使われここの既定値は変わらないこと、推奨の初期値 350 / 500 / 1500 と、復元ボーナスの進行は
  複数の武器で共有され別の武器の作成でも進められるため高めにしていること、大小関係は自由であることを示す
- 入力は下書きであり、「既定値を保存」を押したときだけ保存する。各値は1以上の整数とし、空欄・0・負数・
  小数は該当欄の直下に「1以上の整数を入力してください」と表示して保存できない。3値の大小関係は検証しない。
  保存値から変更が無い間は保存ボタンを無効にし、未保存の変更がある間はその旨を表示する
- 「推奨値に戻す」は下書きを 350 / 500 / 1500 に戻すだけで保存しない
- 保存成功は「探索量の既定値を保存しました。次に候補検索画面を開いたときから使用されます。」、保存失敗は
  「探索量の既定値を保存できませんでした。保存済みの既定値は変更されていません。再度お試しください。」と
  表示し、失敗時は下書きを保持する。読み込み失敗時は値を推測表示せず、読み込めなかったことを表示する
- 保存はDebug Mode保存と同じAppSettings書き込みとして扱う。保存中はData Transferを開始せず、Data Transfer中は
  保存できない（入力も無効）。保存中は二重保存を防ぐ。Debug Mode保存とは対象fieldだけを書き込むため、
  同時に保存しても互いの変更を上書きしない
- Import / Clear成功後は、Importでは `root.settings.candidateSearchDefaults`、ClearではServiceが返した
  default AppSettingsの値（350 / 500 / 1500）で入力を置き換える。入力中の未保存の下書きは破棄する
...
- 実行中の生産計画、ゲーム内セーブ地点、作成中状態、目標武器の完了状態もExport / Importの対象である
  ...
  このExport / Importで行う
- 未対応の `schemaVersion` は拒否する。対応する旧 `schemaVersion`（6..11）は
  [DATA_MODEL.md](./DATA_MODEL.md) 15.3の純粋migrationを順に通してcurrent schema（12）へ変換してから
...
- 最後のゲーム内セーブ地点へ戻す: 12.8の復元を行ってから操作を続ける。再計画の採用だけは、復元後に
  採用を中止して再試算を求める。生産計画画面の競合解消（この候補を優先）による再計算結果の保存も、復元後に
  保存を中止し、「最後のゲーム内セーブ地点へ戻しました。復元前の計算結果は保存していません。復元後の状態から、
  もう一度再計算してください。」と示して再計算を求める（計算結果は復元前の状態のものであるため。
  [PLANNER_SPEC.md](./PLANNER_SPEC.md) 9.2.18 / 16.10）。このときPlanは破棄せず、新しい下書きを開かない
- キャンセル: 何も変更しない
...
- 生産計画画面の競合解消による再計算結果の保存（作成リストの候補置換、[PLANNER_SPEC.md](./PLANNER_SPEC.md) 9.2.18）では、
  警告に「この再計算では作成リストの候補が置き換わり、実行中の生産計画が参照している候補が削除されます。」を添え、
  三択の説明とゲーム側復元ダイアログの説明を「セーブ地点へ戻すだけで、この変更（戻す前の状態で計算した結果）は保存しません。
  生産計画は破棄しません。」の趣旨に差し替える（共通Dialogの操作別説明。新しいDialogは追加しない）
