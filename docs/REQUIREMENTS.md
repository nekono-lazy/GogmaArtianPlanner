...
- ビルドリスト画面の「最大計画ステップ数」の初期表示は、登録されている候補の必要操作数
  （`candidateSnapshot.estimatedOperationCount`）の最大値に、候補の完成時にPlannerが行う確保action
  （`reserve_candidate`）の1を加え、500刻みで切り上げた値とし、最低1000とする。必要操作数1000の候補なら
  1500、1470なら1500、1500なら2000になる。
  ユーザーが手動で変更した後は、再読み込み以外で自動的に書き換えない。「既定値に戻す」は、その時点の
  ビルドリストから同じ規則で求めた値へ戻す（Issue #130）
- 生産計画画面で競合の優先候補を選んで再計算する場合、最大計画ステップ数はビルドリスト画面の
  入力値を引き継がず、表示中の生産計画のStep数を500刻みで切り上げてさらに500を加えた値（最低1000）
  を使う。この上限でも完成しなかった場合は保存せず、ビルドリスト画面から生産計画を作り直すよう案内する
  （Issue #130）
- 最大計画ステップ数は計算時だけの設定であり、生産計画、アプリ設定、IndexedDB、Export / Importの
  いずれにも保存しない
...
### 23.1 競合repair（Issue #136 / #101、Phase 3まで部分実装）
...
[PLANNER_CONFLICT_REPAIR_DESIGN.md](./PLANNER_CONFLICT_REPAIR_DESIGN.md)。代替探索の土台（Search側の探索API。
探索範囲内の理想品ルートを同じ結果の後方位置・Bonus × Skillの組み合わせ・すべての通常アーティア起点まで決定的な順序で
返す探索）はPhase 1として、優先した候補（とそれまでに優先した候補）が使う位置・所持武器を避けてその位置を跨ぐ探索、
見つかった代替を一時的な作成リスト項目にして計画全体を再計算し、代替として成立するかを判定する計算（Domain内の共通計算）は
Phase 2として実装済みである。Issue #101の実ケース（龍を優先）では、火の代替「通常アーティア1本（Normal 0）→ Skill 342で
巨戟化 → Gogma 56以降で復元ボーナス再抽選」が見つかり、龍と両立する計画になることを確認している。
探索範囲と試行・再計算の上限の既定値は、Phase 3の実測をもとに確定した（探索範囲: 通常アーティア4 / 復元ボーナス235 /
スキル4、試行上限: 1目標武器あたり2候補 / 1回の操作あたり計画全体の再計算8回。まだ画面や計算経路には接続していない）。
「比較する」の計算手順（代替を合成した計画の求め方、再計算回数の数え方、表示する件数の意味）はPhase 4-Aで仕様として
確定し、Phase 4-Bで「比較する」の計算をDomain / Workerへ実装した（上記の既定値もWorker内で渡す）。ただし画面の
「比較する」はまだこの計算を呼ばない。「この候補を優先」への適用、履歴の保存、画面からの切替は未実装であり、画面から
使われる現在の実装は上記23章の制約付き再検索のままである。切替はPhase 5で行う。

- 「この候補を優先」は、競合位置1か所の指定ではなく **ルート単位の決定** とする。優先した候補の相手側
  （別の目標武器）の現在のルートは、優先した候補のルートと同時には実行しない
- 相手側の目標武器について、優先した候補（と、ユーザーがそれまでに優先した候補）のルートが使う位置を避けた
  理想品ルートを探す。優先した候補が必ず使う位置には相手側の操作を置かないが、優先した候補の操作で進む位置は
  武器の状態を保ったまま跨いでよい。例えば優先した候補がSkill 341で巨戟化するなら、相手側は342以降で巨戟化
  できる。Counterを進めるだけの途中の通常アーティア作成が使う位置は避けなくてよい（19章、Issue #129）
- 優先した候補のルートが排他的に使う所持武器は、相手側の起点にしない。これは目標武器の優先起点
  （9章、候補の並び順の希望）とは別の条件である
- この代替探索は、初回候補検索と同じ仕組み（探索の進め方、予測、候補の組み立て、候補の同一性）を使うが、
  最初の理想品で止まらず、使えなければ次の理想品へ進む。初回候補検索だけの省略（#104の通常アーティア起点の
  削減など）は、代替探索でも安全だと別に確かめるまで使わない。探索範囲は候補検索の3つの上限と同じ意味の値で
  決め、既定値は実装後の実測で決める。旧制約付き再検索の探索上限を広げる方式は採らない
- 「比較する」は **1段先まで** のpreviewとする。代替ルート（「通常アーティア1本 → 巨戟化 → Reset …」と説明できる
  内容）、その操作量と進行量、代替を採用した場合に残る競合と新しく発生する競合を示し、新しい競合をさらに解決した
  結果までは求めない。保存しない
- 「比較する」は、代替を差し替えて1段解決した計画全体の手数（その計画の実際の手順数）も示し、「Aを優先した場合」と
  「Bを優先した場合」を計画全体の手数で比べられるようにする。優先した候補と代替ルートの操作数を単純に足した値は
  使わない（同じ操作の共有や途中作成の省略などで実際の手順数と一致しないため）。この手数は1段解決した時点の暫定値であり、
  新しい競合が残る場合は、全目標武器が完成するまでの確定値ではない。残る競合・新しい競合・この計画で作成しない
  目標武器と合わせて示す
- 「この候補を優先」は「比較する」と同じ探索を使う。代替が見つかれば相手側の作成リスト項目をそれで置き換え、
  最新の状態から競合を作り直して計画を保存する。古い競合一覧から解決済みだけを消す方式にはしない。代替ルートが
  別の目標武器と新しく競合しても、その場では再帰的に解決せず、保存した計画で次の判断として提示する
- 代替が見つからない場合は、相手側の作成リスト項目を消さず、その目標武器をその計画では作成しないものとして
  理由を示す。相手側の古いルートと優先した候補の間の競合は、選択済みとして扱い、判断待ちとして残さない
- 同じ一連の競合解決の中で一度外したルートを、同じ目標武器の代替として再び採用しない（元のルートへ戻る、
