...
通常アーティアCounterは、武器種とレア度の組ごとに独立して管理する。各Counterについて確定状態と観測回数を保持できること。
...
- Counterが未確定の武器種・レア度について、通常アーティア経由の候補検索を行わない
- Counterが未確定でも、既存巨戟アーティア経由の候補検索は利用可能とする
- すべての武器種・レア度のCounterを事前に確定させる必要はない
...
所持している巨戟アーティアを1本ずつ個別管理する。素材用武器も省略せず登録できること。
...
通常アーティアを作成し、有望な復元構成を利用して巨戟化する。v1では必要に応じたスキル再付与までを同一Routeに含める。
...
2. 必要な武器種・レア度の通常Counterを観測結果から確定する
3. 所持巨戟アーティアを登録する
