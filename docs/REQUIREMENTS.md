...
作成リストは、目標武器ごとにユーザーが現在採用した作成ルートを1件だけ持つ（次期契約。
実装状態は未実装で、Issue #103のPhase 0で実装する。契約本文は[DATA_MODEL.md](./DATA_MODEL.md) 9.4.1）。
...
現行Productionは同じ目標武器に複数の作成リスト項目を保持でき、Plannerがその中から採用する
ルートを決める。
