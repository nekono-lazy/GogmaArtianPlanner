...
### 23.1 競合repair（Issue #136 / #101、Phase 1-Bまで部分実装）
...
優先した候補が使う位置の予約が無い場合の基本探索）はPhase 1-Bまで実装済みだが、探索の完全性、優先した候補が使う
位置・所持武器を避ける探索、計画の再計算との接続、「比較する」「この候補を優先」への適用は未実装である。
