...
### 23.1 競合repair（Issue #136 / #101、Phase 1まで部分実装）
...
優先した候補が使う位置の予約が無い場合に、探索範囲内の理想品ルートを同じ結果の後方位置・Bonus × Skillの組み合わせ・
すべての通常アーティア起点まで決定的な順序で返す探索）はPhase 1として実装済みだが、優先した候補が使う位置・所持武器を
避ける探索（Phase 2）、計画の再計算との接続、「比較する」「この候補を優先」への適用は未実装である。
画面から使われる現在の実装は上記23章の制約付き再検索のままであり、切替はPhase 5で行う。
