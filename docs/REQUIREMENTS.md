...
実装状態（Phase 0-1時点）: 計画作成の停止（同じ目標武器の項目が複数ある場合）、置き換えの
Domain / Service基盤、既存データの保持は実装済みである。検索画面の置き換え確認と作成リストでの
整理の案内（Phase 0-2）、制約付き再検索での置き換え（Phase 0-3）は未実装であり、それまで検索画面は
置き換えが必要な追加を何も書かずに失敗として伝え、制約付き再検索の採用は元の項目の横に追加する。
