...
[DATA_MODEL.md](./DATA_MODEL.md) 9.4.1。実装状態は一部実装で、Issue #103のPhase 0で段階的に実装する）。
...
実装状態（Phase 0-2時点）: 計画作成の停止（同じ目標武器の項目が複数ある場合）、置き換えの
Domain / Service基盤、既存データの保持（Phase 0-1）と、検索画面の置き換え確認および作成リストでの
整理の案内（Phase 0-2）は実装済みである。制約付き再検索での置き換え（Phase 0-3）は未実装であり、
それまで制約付き再検索の採用は元の項目の横に追加する（作成リストでは整理の案内が出る）。
