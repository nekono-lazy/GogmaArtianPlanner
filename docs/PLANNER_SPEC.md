...
// Production Planner（決定的scheduler）の唯一のbound（Issue #103 Phase D-2a）
...
Issue #103 Phase D-2aで `PlannerOptions` をProduction schedulerが実際に読む `maxPlanSteps` だけへ
縮小した。Beam Search oracleの `beamWidth` / `maxExpandedStates` はProductionの型・既定値・
validation・Worker requestから外し、oracle専用contract（7.2.2）へ分離した。
...
- `targetWeapons` はPlannerへ渡すplanning inputのTarget集合である。Entryの参照先解決、
  ...
  理由を返す
- `maxPlanSteps` は1以上の整数（Production `validatePlannerOptions()`）。Beam Search oracleの
  `beamWidth` / `maxExpandedStates` はProductionのvalidation対象ではなく、oracle専用の
  `validatePlannerBeamSearchOptions()` が検証する（7.2.2）
...
`max_steps_reached` は `maxPlanSteps` に到達した場合だけ使用する。Production Planner（決定的scheduler）の
boundは `maxPlanSteps` だけである。Beam Search oracleの `maxExpandedStates` 到達を表すwarning kindは無い:
Issue #103 Phase D-2bで `max_expanded_states_reached` を削除し、oracleのboundはtypedな
`PlannerBeamSearchTermination.reachedLimits`（`max_expanded_states`）だけで表す（7.2 / 7.2.2）。
...
`PlannerSearchAction` 列であり、Production中立の結果型 `PlannerRunResult` を返す（Issue #103
Phase D-2a、7.2.1）。Beam Search oracleは同じfieldを持つ専用の `PlannerBeamSearchResult` を返す（7.2.2）。
Trace Replayを省略しない。
...
- （Beam Search oracle）`maxExpandedStates = N` の場合はN件まで許可し、N+1件目を構築しない。
  N件へ到達したことは `PlannerBeamSearchTermination.reachedLimits` の `max_expanded_states` だけで表し、
  warningは返さない（Issue #103 Phase D-2b）
...
- 決定的schedulerは `max_expanded_states`（`reachedLimits`）を発生させない
- `expandedStates` の計測値は診断のため従来どおり数え、
  `PlannerRunTermination.expandedStates` と `PlannerRunResult.expandedStates` で返す。
  Production Workerは数値progressを返さない（Issue #103 Phase D-2a、14章）。schedulerにも
  Beam Search oracleにも数値progress callbackは無い（Issue #103 Phase D-2b）
...
Production full Planner runの終了状態は `PlannerRunTermination` としてtypedに返す
（Issue #103 Phase D-2a）。Production schedulerのboundは `maxPlanSteps` だけなので、
`reachedLimits` は `max_plan_steps` しか取り得ず、`limits` はProduction `PlannerOptions`
（`{ maxPlanSteps }`）だけである。Production terminationが `max_expanded_states` を持てないことは
型で保証する。Beam Search oracleは同じstatus・同じ決定順序の専用型
`PlannerBeamSearchTermination` を返す（7.2.2）。両者は共通の
`PlannerTerminationOf<TLimitKind, TLimits>` を共有し、status導出は `createPlannerTermination()`
1つだけが行う。
...
export interface PlannerTerminationOf<TLimitKind extends string, TLimits> {
  ...
  limits: TLimits;
  expandedStates: number;          // 診断値。Production boundでも進捗分母でもない
  completedTargetCount: number;
  totalTargetCount: number;
}

export type PlannerRunTermination =
  PlannerTerminationOf<PlannerRunLimitKind, PlannerOptions>;
```
...
3. `incomplete`: それ以前にboundが探索を打ち切った（Productionでは `maxPlanSteps` だけ）
...
| `incomplete` | `maxPlanSteps` が完成前にscheduleを打ち切った（唯一のProduction bound） |
...
`max_steps_reached` PlannerWarningは診断情報として残す。Beam Search oracle専用だった
`max_expanded_states_reached` はIssue #103 Phase D-2bで削除した。oracleのboundは
`PlannerBeamSearchTermination.reachedLimits` だけで表し、warningとの二重表現はしない。
...
- UIは到達したbound（`max_plan_steps`）、設定値、完成Target数、設定見直し案内を表示する。
  「探索状態数 x / y」は表示しない（UI_FLOW 10.1）。Production表示helperは `max_expanded_states`
  分岐を持たない
- Persistenceのエラー文言は「Planner run did not complete: it reached max_plan_steps ...」のように
  中立で、Beam Search前提の表現を持たない。拒否判断（`termination.status === "incomplete"`）は不変
...
#### Calculation compatibility（Issue #103 Phase D-2a）

Phase D-2a（Production型 / termination / Worker protocolの整理）も
`CURRENT_CALCULATION_APP_SCHEMA_VERSION` を **14のまま**据え置く。

- `PlannerOptions` は `PlanningInputSnapshot` にも他のpersisted entityにも保存されていない
  runtime入力であり、`beamWidth` / `maxExpandedStates` を型から外しても保存済みデータの意味は変わらない
- `ProductionPlan` / `PlanStep` / `PlanningInputSnapshot` / `CalculationContext` / DB / Exportの
  persisted shapeは変更しない
- schedulerのaction選択、canonical ordering、Route commitment、Conflict、deadlock / stall、
  Plan projectionのsemanticsは変更しない。同じ入力からは同じPlanが得られる
- Worker protocol（progress response削除、Client callback削除）はアプリ内部のruntime境界であり、
  永続artifactを生成しない
...
export const defaultPlannerBeamSearchOptions = {
  maxPlanSteps: 1000,
  beamWidth: 50,
  maxExpandedStates: 10000,
};

export type PlannerBeamSearchInput =
  Omit<PlannerInput, "options"> & { options: PlannerBeamSearchOptions };

export type PlannerBeamSearchLimitKind = PlannerRunLimitKind | "max_expanded_states";
export type PlannerBeamSearchTermination =
  PlannerTerminationOf<PlannerBeamSearchLimitKind, PlannerBeamSearchOptions>;
export type PlannerBeamSearchResult = PlannerRunResultOf<PlannerBeamSearchTermination>;

// Issue #103 Phase D-2b: Productionと同じ shouldCancel / yieldControl だけ
export type PlannerBeamSearchExecutionOptions = PlannerExecutionOptions;
```

- oracleは `maxPlanSteps` / `beamWidth` / `maxExpandedStates` の3つを使い、Beam Searchの展開・評価・
  枝刈り・停止のsemanticsはD-2a前と同じである
...
- Issue #103 Phase D-2bの判断（`docs/ISSUE_103_DETERMINISTIC_PLANNER_DESIGN.md` 14.6）: Beam oracle、
  `comparePlannerSearchStates`、semantic key、`evaluationScore`、`totalCost`、
  `preferredSourceProgressCount` はregression oracleとして保持し、Beam / scheduler parity harness、
  acceptance catalogue、`sanity-3` / `representative-12` parityはCI regressionとして保持する。
  Issue #103 Browser benchmark（page / Worker / controller / Node runner）、PR #107 Beam instrumentation、
  Beamの `onProgress` は削除した。schedulerのinstrumentationは `drops` / `provisionalOutcomes` を
  報告するparity用test observer（`PlannerScheduleExecutionOptions.schedulerInstrumentation`）へ縮退した
...
**Phase 5-B以降のrouting（Issue #136 / #101）。** 生産計画画面の「比較する」は9.2.19.7のPlanner Alternative what-if
（`PlannerWorkerClient.createPlannerAlternativeComparison()`。boundsをrequestに持たず、表示中Draftの
`conflictRepairLineage` から導いたprior fixed Entry / prior除外Route keyを渡し、`PlannerInput.options` は
`conflictResolutionPlannerOptions(表示中Plan)`）、「この候補を優先」は9.2.19.8のactual repair
（`PlannerWorkerClient.createPlannerAlternativeRepair()` とPlanner Alternative専用Persistence、9.2.15末尾）である。
本節のうち表示authority、explicit resolutionの復元、participantのcurrent availability、request lifecycle（requestId /
generation / cancel）、what-ifの非永続とexplicit choiceの独立、`status === 'draft'` だけを編集対象とする規則、
...
`breaking_change_approved`、「最後のゲーム内セーブ地点へ戻す」ではセーブ地点復元だけを書き、artifactのEntry・Draft・lineageを
...
を1つのDexie read-write transactionで行い、Entryだけ・Planだけ・lineageだけのpartial saveを残さない。
...
既存の `max_steps_reached` は1回のfull Planner runの `PlannerOptions` boundを表すものであり、
意味が異なるため流用しない（Beam oracle専用だった `max_expanded_states_reached` はIssue #103
Phase D-2bで削除した）。
...
Phase 5-Aで実装した。Phase 5-B（lineage永続化とmigration、Persistence、Worker / Client、Production routing切替とversion更新）も
実装済みである: `ProductionPlan.conflictRepairLineage`（[DATA_MODEL.md](./DATA_MODEL.md) 11.1.1、Dexie v10 / Export 13で既存Plan本体へ
`null` を補う）、actual repairのWorker request kind（`create_planner_alternative_repair` / `create_planner_alternative_repair_result`）、
Production Worker adapter（`createProductionPlannerAlternativeRepair()`、what-ifと同じProduction extent / 試行上限をWorker境界内で渡す）、
`PlannerWorkerClient.createPlannerAlternativeRepair()`、Planner Alternative専用のPersistence（9.2.15末尾）、そして生産計画画面の
...
（`ProductionPlan.conflictRepairLineage`、[DATA_MODEL.md](./DATA_MODEL.md) 11.1。Phase 5-Bで追加）に保持する。
...
- 試行上限のProduction default値はPhase 3-Cで次のとおり確定した（Planner Domain、
  ...
  - Domain API（`runPlannerAlternativeKernel()` の `bounds` / `extent`、`createPlannerAlternativeFullRunBudget()`）は
    ...
    Phase 5-Bで、actual repairのProduction Worker adapter `createProductionPlannerAlternativeRepair()` も同じ2定数を
    Worker境界内で明示的に渡すよう配線した）
...
- **Phase 5-B**（actual repairの新semanticsのProduction routing切替、lineage永続化。**実装済み**: 下記のとおり
  `CURRENT_CALCULATION_APP_SCHEMA_VERSION` 16、`DATABASE_SCHEMA_VERSION` 10、`ExportRoot.schemaVersion` 13、
  build-result例外 `16 -> [12, 13, 14, 15]` へ更新した）
...
- full Planner runが `maxPlanSteps` へ到達した時は `max_steps_reached` を返す
...
`error` は両request kindで共有する（Issue #103 Phase D-2aで `progress` responseを削除した）。
...
`cancel` はgenerationを運び、両result・`error` はそれをechoする。Clientが
...
ordinary / constrained / what-ifは同じ `requestId` / generation、`cancel`、
...
schedulerになる。Worker requestへstrategyを追加しない。Phase Bの計測に使ったbenchmark専用Worker
（Issue #103 benchmark）の `strategy` は計測用でありProduction strategy flagではなかった。このWorkerは
Issue #103 Phase D-2bで削除した。

Issue #103 Phase D-2aでProduction Worker protocolを整理した。

- Production Planner Workerは `type: "progress"` responseと `PlannerProgress` DTOを持たない。
...
- request / resultの `PlannerInput.options` と `termination` はProduction型（`maxPlanSteps` /
  `max_plan_steps` だけ）であり、`beamWidth` / `maxExpandedStates` を運ばない
- testの観測hook（schedulerのparity用observer `schedulerInstrumentation`）はscheduler専用の
  `PlannerScheduleExecutionOptions` だけにあり、Production Worker protocol / Clientへ露出しない。
  schedulerとBeam oracleの `onProgress` とBeamの `searchInstrumentation` はIssue #103 Phase D-2bで
  削除した。benchmarkのためにProduction Workerのprogress responseを残さない
...
- （Phase D-2a）`defaultPlannerOptions` は `{ maxPlanSteps: 1000 }` だけであり、Production
  `PlannerOptions` は `beamWidth` / `maxExpandedStates` を型として持たない。`validatePlannerOptions()` は
  `maxPlanSteps` だけを検証する
- （Phase D-2a）Production terminationの `reachedLimits` は型上 `max_plan_steps` だけであり
  （`max_expanded_states` を代入するとcompile error）、Beam oracleのtermination / resultは
  Production型へ代入できない
- （Phase D-2a）runtimeに紛れ込んだoracle field（`beamWidth` / `maxExpandedStates`）をschedulerは
  読まず、Production結果とterminationの `limits`（`{ maxPlanSteps }`）へ反映しない。同じ入力の
  Beam Search oracleは自身の `maxExpandedStates` で従来どおり止まる
- （Phase D-2a）Production Worker（通常Plan / constrained / what-if）は `type: "progress"` を送信せず、
  calculationへ `shouldCancel` / `yieldControl` だけを渡す。Worker Clientはcallbackを持たず、
  `cancelPlan()` とstale generation無視の契約を維持する
- （Phase D-2a）Production moduleが `PlannerBeamSearch*` 型、`defaultPlannerBeamSearchOptions`、
  `PlannerProgress`、`beamWidth`、`maxExpandedStates` をコード上参照しない
...
- typed terminationがfull Planner run結果（Productionでは `PlannerRunResult`）からProduction Plan生成、
  orchestration結果、Worker応答、Worker Clientまで再構築されずに届く
- （Phase D-2a）oracle専用の `validatePlannerBeamSearchOptions()` が `beamWidth` /
  `maxExpandedStates` の不正値でoracle入力をfail closedし、`defaultPlannerBeamSearchOptions` は
  1000 / 50 / 10000である
...
- 実ユーザーケース（Bonus 23 / Bonus 148 + Skill 82の2 Target）がBeam Search oracleの既定上限で
  `incomplete` と24 step partialになり、Production Plan生成（決定的scheduler、`maxPlanSteps = 300`）
  では230 stepの完成Planになる
...
### 15.9.2 Planner Alternative Search / 1段repair Test（9.2.19、Phase 5まで実装）
...
`planner.worker.production.alternative.test.ts`、`plannerWorkerClient.test.ts`）。actual repair / 保存時の決定の展開の
Pure Domain項目はPhase 5-Aで、lineage永続化とmigration / Planner Alternative専用Persistence（0 / 1 / 複数replacement、
非選択のaccepted replacement、保存時の `O` 変更・generated ID衝突・metadata / lineage不正・参照不整合のfail closed、
Plan-breaking guard）/ actual repair Worker・Client / 生産計画画面のrouting切替とrepair chain / version境界の項目はPhase 5-Bで
実装した（`plannerResultPersistenceService.alternative.test.ts`、`conflictRepairLineageMigration.test.ts`、
`conflictRepairLineageExportMigration.test.ts`、`conflictRepairLineageValidation.test.ts`、`calculationSchema16Boundary.test.ts`、
`ProductionPlanPage.test.tsx`、`issue101PlannerAlternativeRepair.test.ts`）。
