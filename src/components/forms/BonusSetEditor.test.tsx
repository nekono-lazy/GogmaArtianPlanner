...
function EditorFixture({ empty = false }: { empty?: boolean }) {
  ...
  return <BonusSetEditor label="復元ボーナス" master={master} weaponTypeId={weaponTypeId} scope="gogma_artian" value={value} onChange={setValue} />
}

function FormalEditor({ weaponTypeId }: { weaponTypeId: string }) {
  const result = loadMasterData()
  if (!result.ok) throw new Error('Master load failed')
  const [value, setValue] = useState(
    createDefaultBonusSet(result.data, weaponTypeId),
  )
  return (
    <BonusSetEditor
      label="復元ボーナス"
