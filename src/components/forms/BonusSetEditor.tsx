...
export function BonusSetEditor({ label, master, weaponTypeId, value, onChange }: BonusSetEditorProps) {
  ...
  if (typeIds.length === 0) return <Alert severity="error">{label}: 復元ボーナスMaster Dataが利用できません。</Alert>
  ...
  return <Stack spacing={1}>
    ...
    {value.map((bonus, index) => {
      ...
      return <Stack key={index} direction={{ xs: 'column', sm: 'row' }} spacing={1}>
        <FormControl fullWidth><InputLabel id={`${label}-${index}-type`}>枠{index + 1} Bonus Type</InputLabel><Select labelId={`${label}-${index}-type`} label={`枠${index + 1} Bonus Type`} value={bonus.bonusTypeId} onChange={(event) => update(index, event.target.value)}>{typeIds.map((id) => <MenuItem key={id} value={id}>{master.bonusTypes.find((type) => type.id === id)?.displayNameJa ?? '不明'}</MenuItem>)}</Select></FormControl>
        <FormControl fullWidth><InputLabel id={`${label}-${index}-rank`}>枠{index + 1} Rank</InputLabel><Select labelId={`${label}-${index}-rank`} label={`枠${index + 1} Rank`} value={bonus.bonusRankId} onChange={(event) => update(index, bonus.bonusTypeId, event.target.value)}>{ranks.map((rank) => <MenuItem key={rank.id} value={rank.id}>{rank.displayNameJa}</MenuItem>)}</Select></FormControl>
      </Stack>
    })}
  </Stack>
}
