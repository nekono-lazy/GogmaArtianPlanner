...
export function BonusSetEditor({ label, master, weaponTypeId, scope, value, onChange }: BonusSetEditorProps) {
  const definitions = master.weaponBonusDefinitions.filter(
    (definition) => definition.isEnabled && definition.weaponTypeId === weaponTypeId && definition.scope === scope,
  )
  ...
  const update = (index: number, bonusTypeId: string, bonusRankId?: string) => {
    const ranks = getRanksForBonusType(master, weaponTypeId, bonusTypeId, scope)
  ...
  return <Stack spacing={1}>
    ...
    {value.map((bonus, index) => {
      const ranks = getRanksForBonusType(master, weaponTypeId, bonus.bonusTypeId, scope)
