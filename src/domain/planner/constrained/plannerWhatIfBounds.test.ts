...
describe('B9-B1a Production default', () => {
  /**
   * PLANNER_SPEC 9.2.4.9 forbids a B9 Production default until a real Browser
   * Worker benchmark decides one, and forbids reusing
   * `defaultPlannerOrchestrationBounds` or `CandidateSearchSettings` as a
   * substitute. The absence is asserted on the public Domain surface.
   */
  it('exports no what-if bounds default', () => {
    expect('defaultPlannerWhatIfBounds' in constrainedModule).toBe(false)
    expect(
      Object.keys(constrainedModule).filter(
        (key) => key.startsWith('default') && key.includes('WhatIf'),
      ),
    ).toEqual([])
  })
