...
import { createCandidateSearchInput, createCandidateSearchEngine, belowPracticalBonuses } from '../../test/fixtures/candidateSearch'
...
import { buildBonusSolutionSet, buildSkillSolutionSet, selectBonusAxis, selectSkillAxis } from './streamSolutions'
...
describe('Ideal-only Search', () => {
  it('has no Practical axes even when Ideal matches; no Practical horizon is scheduled', () => {
    ...
    expect(selectBonusAxis(bonus, 'ideal')).toHaveLength(1)
    expect(selectSkillAxis(skill, 'ideal')).toHaveLength(1)
    expect(selectBonusAxis(bonus, 'practical')).toEqual([])
    expect(selectSkillAxis(skill, 'practical')).toEqual([])
  })
  it('retains the canonical Ideal, never predicts beyond it, and repeats deterministically', async () => {
    ...
    for (const searchRunId of ['ideal-only-a', 'ideal-only-b']) {
      ...
      const candidates = result.targetResults.flatMap((target) => target.candidates)
      expect(candidates).toHaveLength(1)
      expect(candidates[0]).toMatchObject({ category: 'ideal', conditionMatch: { bonus: 'ideal', skill: 'ideal' } })
