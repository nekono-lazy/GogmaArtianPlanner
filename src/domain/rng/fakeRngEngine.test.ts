...
describe('FakeRngEngine', () => {
  ...
  it('reports prediction support from fixture capabilities without modeling production coverage', () => {
    ...
    expect(engine.getPredictionSupport({ type: 'gogma_keep', weaponTypeId: 'weapon.unknown', elementId: 'element.unknown', currentBonuses: [] as never, master: { weaponBonusDefinitions: [], bonusRanks: [], lotteries: [] } })).toEqual({ supported: false, reason: 'engine_capability_unavailable' })
  })
})
