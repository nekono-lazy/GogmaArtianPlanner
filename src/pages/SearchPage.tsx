...
import { skippedRouteReasonLabels } from '../components/search/searchPresentation'
...
export function SearchPage({ dependencies = defaultDependencies ?? undefined }: SearchPageProps) {
  ...
  const addToBuildList = async (candidate: BuildCandidate) => {
    ...
    if (!dependencies || !target) {
      setError('候補に対応するTargetが見つかりません。')
      return
    }
    try {
      const added = await dependencies.addCandidate(candidate, target)
      setNotice(added.added ? 'Build Listへ追加しました。' : '同等の候補はすでにBuild Listにあります。')
    } catch (caught: unknown) {
      setError(caught instanceof Error ? caught.message : 'Build Listへの追加に失敗しました。')
    }
  }

  return (
    <PageShell title="Search Results" description="目標武器ごとに利用可能な作成候補を検索します。">
      <Stack spacing={3}>
        {loading && <LinearProgress aria-label="検索データを読み込み中" />}
        {error && <Alert severity="error">{error}</Alert>}
        {notice && <Alert severity="info">{notice}</Alert>}
        {!loading && enabledTargets.length === 0 && <Alert severity="info">Target武器を登録してください。</Alert>}
        {enabledTargets.length > 0 && (
          <Paper variant="outlined" sx={{ p: { xs: 2, sm: 3 } }}>
            <Stack spacing={3}>
              <Typography variant="h2">検索条件</Typography>
              <Box>
                <Typography variant="subtitle2">検索対象Target（複数選択可）</Typography>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <FormControl fullWidth><InputLabel id="route-filter-label">Route</InputLabel><Select labelId="route-filter-label" label="Route" value={routeFilter} onChange={(event) => setRouteFilter(event.target.value as CandidateRouteFilter)}><MenuItem value="all">すべて</MenuItem><MenuItem value="normal_artian">通常アーティア経由</MenuItem><MenuItem value="existing_gogma">所持巨戟アーティア</MenuItem></Select></FormControl>
              ...
              <Accordion><AccordionSummary><Typography>詳細設定</Typography></AccordionSummary><AccordionDetails><Stack spacing={2}>
                {(['maxNormalAdvance', 'maxGogmaAdvance', 'maxSkillAdvance', 'maxCandidatesPerTarget'] as const).map((key) => <TextField key={key} label={key} type="number" value={settings[key]} onChange={(event) => updateSetting(key, event.target.value)} slotProps={{ htmlInput: { min: 1 } }} />)}
                <TextField label="similarityThreshold" type="number" value={settings.similarityThreshold} onChange={(event) => updateSetting('similarityThreshold', event.target.value)} slotProps={{ htmlInput: { min: 0, max: 1, step: 0.1 } }} />
