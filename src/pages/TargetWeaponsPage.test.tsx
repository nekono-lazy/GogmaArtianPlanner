...
describe('TargetWeaponsPage', () => {
  it('creates priority-3 Target with five Ideal slots and condition editors', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'Targetを追加' }))
    ...
    await user.click(screen.getByRole('button', { name: '代替Groupを追加' }))
    expect(screen.getByLabelText('Required Count')).toBeInTheDocument()
    expect(screen.getByLabelText('Group Required Count')).toBeInTheDocument()
