...
describe('B8-E1 workload C: shared trial budget and repeated reruns', () => {
  ...
  it('goes further with a larger trial bound than with a small one', async () => {
    ...
    // Constrained enumeration now yields Ideal Candidates only, so this
    // workload adopts one Entry and the enumeration bound stops the remaining
    // work. That is an observation of *this* workload, not a Domain maximum.
    expect(larger.generatedBuildListEntries).toHaveLength(1)
    expect(larger.warnings.map(({ kind }) => kind)).toEqual([
      'constrained_enumeration_bound_reached',
    ])
...
describe('B8-E1 workload D: generated-entry cap', () => {
  ...
  it('adopts one Entry and still reports a stop with a generous cap', async () => {
    ...
    // Nothing here should be read as "at most one generated Entry is
    // possible": the Ideal-only enumeration simply offers fewer adoptable
    // Candidates than the former Practical-and-Ideal one did.
